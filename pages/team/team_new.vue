	...
	<view class="new_con">
		...
		<view class="new_data" v-if="new_data.length>0">
			<view class="new_list" v-for="(item,index) in new_data" :key="index">
				...
				<label>{{item.explain}}</label>
				<view class="operation">
					<block v-if="item.type==1">
						<block v-if="item.state==1">
							<text @click="add_operation(item.id,3)">拒绝</text><text @click="add_operation(item.id,2)">同意</text>
						</block>
...
<script>
	...
	export default{
		...
		methods:{
			new_list(id){
				this.$request(apps.api.team_new,{id:this.id}).then(res=>{
					if(res.code==200){
						this.new_data=res.data;
					}
				})
			},
			add_operation(new_id,state){
				this.$request(apps.api.team_user_add,{new_id:new_id,'groups_id':this.id,state:state}).then(res => {
