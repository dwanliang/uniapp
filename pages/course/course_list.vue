<template>
	...
	<view class="time-table width-100-bfb flex flex-direction" style="height: 100vh">
		...
		<view v-for="(item, index) in course_data " :key="index" v-if="index + 1 === week" class="flex-sub">
			<view class="" v-for="(items, i) in item" :key="i" v-if="item.length !== 0" >
				<view class="cu-card article" :class="isCard ? 'no-card' : ''">
					<view class="cu-item padding-bottom-xs margin-bottom-xs cu-shadow" @click="course_edit(items.id)">
						...
						<view class="text-lg">
							...
							<view style="height: 60rpx;">
								<text class="title padding-right-unset">老师：</text>
								<text>{{ items.courseTeacher }}</text>
							</view>
							<view>
								<text class="title padding-right-unset">地址：</text>
								<text>{{ items.courseAddress }}</text>
							</view>
			...
			<view class="all-add" @tap="course_add()">
...
export default {
	...
	methods: {
		course_edit(id){
			uni.navigateTo({
				url:'../course/course_edit?id='+id
			})
		},
		course_list(){
			this.$request(apps.api.course_list,{}).then(res=>{
				if(res.code==200){
					var i=0;
					for(i in res.data){
						var week=res.data[i].week-1;
						console.log(typeof res.data[i])
						this.course_data[week].push(res.data[i])
		...
		course_add(){
			uni.navigateTo({
				url:'../course/course_add?week='+this.week+''
			})
		},
		tabSelect(e) {
