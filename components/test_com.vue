<template>
	<view class="box">
		<view class="uni-padding-wrap" @click="change">
			<view class="uni-title">{{selectRes}}</view>
		</view>
	...
	export default {
		...
		data() {
			...
			if(this.ke==1){
				...
				value=[nowYear, month - 1, day - 1, hour - 1, minute];//alksd
			}
			return {
				years,
				year,
				...
				selectArr: [],
				showPicker: false,
				selectRes: year+'-'+month+'-'+day+' '+hour+':'+minute,
				dates:this.date,
			};
		},
		mounted(){
		...
		methods: {
			starts(){
				var da=this.dates;
				if(this.ke==1){
					this.selectRes=this.hour+':'+this.minute;
					if(da){//判断是否有默认值传进来
						if(da[0]<10){
							var da1='0'+da[0]+':'
						}else{
						...
						this.selectRes= da1+da2+'';
						this.value=[da[0]-1,da[1]];
					}
				}
				if(this.da){//判断是否有默认值传进来
					this.selectRes= da[0]+'-'+da[1]+'-'+da[2]+' '+da[3]+':'+da[4];
					this.value=[da[0]-this.beginYear,da[1]-1,da[2]-1,da[3]-1,da[4]];
				}
				this.$emit("show", {
			...
			bindChange: function(e) {//滚轮
				...
				if(this.ke==1){
					this.selectRes=this.hours[val[0]]+':'+this.minutes[val[1]];
					if(this.da){//判断是否有默认值传进来
						if(da[0]<10){
							var da1='0'+da[0]+':'
						}else{
